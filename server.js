...
app.get('/setup-admin', async (req, res) => {
  try {
    ...
    const newAdmin = new User({ username: 'admin', password: hashedPassword, role: 'admin', fullName: 'Aerospace Admin' });
    await newAdmin.save();
...
app.post('/api/send-otp', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Error sending email. Check credentials.' }); }
});

app.post('/api/register', async (req, res) => {
  try {
    const { fullName, username, email, password, otp } = req.body;
    if (!otpStore[email] || otpStore[email] !== otp) return res.status(400).json({ success: false, message: 'Invalid or Expired OTP. Please try again.' });
    const hashedPassword = await bcrypt.hash(password, 10);
    const newStudent = new User({ fullName, username, email, password: hashedPassword, role: 'student' });
...
app.put('/api/courses/:id', async (req, res) => {
  try {
    ...
    await Course.findByIdAndUpdate(req.params.id, { $set: { name, description, price, isPremium } });
    res.json({ success: true, message: 'Course updated successfully!' });
  } catch (error) { res.status(500).json({ success: false, message: 'Error updating course.' }); }
});
...
app.put('/api/courses/:courseId/materials/:materialId', async (req, res) => {
  try {
    ...
    await Course.updateOne(
      ...
      { $set: { "materials.$.title": title, "materials.$.description": description, "materials.$.isPremium": isPremium, "materials.$.price": price } }
    );
    res.json({ success: true, message: 'Material updated successfully!' });
  } catch (error) { res.status(500).json({ success: false, message: 'Error updating material.' }); }
});
...
app.delete('/api/courses/:courseId/materials/:materialId', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Server error deleting material.' }); }
});

// Q&A System with Advanced Details
app.post('/api/courses/:id/doubts', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Error submitting doubt' }); }
});

app.put('/api/courses/:courseId/doubts/:doubtId', async (req, res) => {
  try {
    const { answer } = req.body;
    await Course.updateOne({ _id: req.params.courseId, "doubts._id": req.params.doubtId }, { $set: { "doubts.$.answer": answer } });
    res.json({ success: true, message: 'Answer posted!' });
  } catch (error) { res.status(500).json({ success: false, message: 'Error posting answer' }); }
});
...
app.post('/api/create-order', async (req, res) => {
  try {
    ...
    const order = await razorpay.orders.create({ amount: amount * 100, currency: "INR", receipt: "aero_receipt_" + Math.random().toString(36).substring(7) });
    res.json({ success: true, order });
  } catch (error) { res.status(500).json({ success: false, message: 'Server error creating order' }); }
});
...
app.post('/api/verify-payment', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Verification error' }); }
});
